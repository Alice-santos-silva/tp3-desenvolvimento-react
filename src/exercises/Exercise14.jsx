...
function Exercise14() {
  ...
  const onSubmit = async (data) => {
    if (selectedRow) {
      try {
        await updateDoc(doc(db, 'formularios', selectedRow.id), data);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: row => (
        <>
          <button onClick={() => handleRowSelected(row)}>Editar</button>
  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit(onSubmit)}>
        ...
        <button type="submit">{selectedRow ? 'Atualizar' : 'Enviar'}</button>
      </form>
